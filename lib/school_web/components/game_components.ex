defmodule SchoolWeb.GameComponents do
    ...
    <div class="leaderboard">
      ...
      <ul class="leaderboard-list">
        <li :for={player <- @player_list} class="leaderboard-item">
          ...
          <%= if @local_player && @local_player.name != player.name && @local_player.ezic_score >= 5 do %>
            <button
              phx-click="punish_player"
              phx-value-name={player.name}
              style="width: 24px; height: 24px; border-radius: 4px; background-color: var(--paper-line); border: 1px solid var(--ink-muted); margin-left: 8px; cursor: pointer; transition: background-color 0.2s; display: inline-flex; align-items: center; justify-content: center; font-size: 14px; font-weight: bold; color: var(--ink-soft);"
            >
              X
            </button>
          <% end %>
  ...
  def punish_overlay(assigns) do
    ...
    <div class="match-end-overlay" style="display:flex">
      <div
        class="match-end-card"
        style="background: var(--danger-bg); border: 2px solid var(--stamp-red);"
      >
        ...
        <p style="margin-bottom: 24px; color: var(--ink); font-size: 14px; font-weight: 500;">
          A new rule has been added to your regulations.
        </p>
        <button
          phx-click="close_punish_popup"
          class="btn-new-match"
